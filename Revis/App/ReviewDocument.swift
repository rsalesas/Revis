...
struct ReviewFile: Codable, Equatable, Sendable {
    ...
    ///
    /// The "how to apply" paragraph is `ReviewPolicy.rules`, not a second telling of the
    /// same rules — this is the one file `ReviewPolicy`'s own comment does not name as a
    /// reader, because when this was hand-written the file format had nothing else in
    /// common with `ReviewExport` to build it from.
    static let aiGuidance: String = {
        let howToApply = ReviewPolicy.rules.enumerated()
            .map { "\($0.offset + 1). \($0.element)" }
            .joined(separator: "\n")
        return """
            This is a Revis review: a snapshot of a document plus what a reviewer marked \
            on it, from the Revis app (a sibling of vaelora.app, for reviewing documents \
            written by a language model). If the "revis-review" Claude Code skill is \
            available, use it — it goes further than this note. Otherwise:

            `annotations` is the review. Each one has an `intent` (change / insert / \
            remove / move / question / comment), a `note` (the instruction), and an \
            `anchor` — an address made of text, not position: `anchor.quote` (plus \
            `anchor.prefix` / `anchor.suffix` for the words either side, where the quote \
            is short or repeated) and `anchor.path` for where a person would say it is.

            How to apply one, in order:
            \(howToApply)

            Of what's left after that: change / insert / remove / move is an edit to \
            make; `question` wants an answer, not an edit; `comment` is an observation \
            with no action requested, and one carrying `verdict: "approved"` should be \
            left exactly as it stands. Skip anything with `verdict: "declined"` — a \
            reviewer turned it down, and acting on it anyway makes a change somebody \
            explicitly refused — and anything with `status: "resolved"`, which has \
            already been dealt with.

            `document` is the full sanitized snapshot of what was reviewed — what the \
            annotations are anchored into, not something to read start to end. If a \
            person handed you this file directly, Revis's own File \u{25b8} Export \
            (Markdown or JSON) already does the quote-matching for a Markdown document \
            and is worth asking for instead of working from this file.

            This file format has no reply mechanism of its own. Revis reads a reply \
            document back through File \u{25b8} Import Replies\u{2026}: Markdown, one \
            heading per annotation id (the id printed beside the item, never its \
            position), the reply underneath.
            """
    }()
