...
struct DocumentWebView: NSViewRepresentable {
    ...
    var tool: ReviewTool
    /// Whether the DESK behind the sheet is drawn dark. Pushed rather than sensed: the web
    /// view is pinned to a light appearance below, so the page can no longer ask.
    var deskIsDark: Bool
    ...
    func makeNSView(context: Context) -> WKWebView {
        ...
        /* The GUEST document is laid out in a light world, always.
         *
         * `prefers-color-scheme` inside a `WKWebView` follows the view's effective
         * appearance, so on a Mac in Dark Mode a document with a dark branch took it — and
         * the sheet it is drawn on is paper-white whatever the reviewer's Mac is set to,
         * deliberately (see review.css). The result was the document's dark surfaces
         * painted onto white paper: a masthead at #151D1A carrying a heading the sheet had
         * already darkened to #1C1C1E. Black on black, and the document's own light palette
         * — the one that matches the paper it is being shown on — never asked for.
         *
         * Pinning the view to Aqua is the whole fix, and it is one line because the
         * question was never "which colours should we force" but "which world is this
         * document being read in". The desk is the only thing here that legitimately
         * follows the reviewer, and it is told: `rvSetDesk`, below. */
    ...
    func updateNSView(_ webView: WKWebView, context: Context) {
        ...
        if c.lastDeskIsDark != deskIsDark {
            c.lastDeskIsDark = deskIsDark
            c.run("window.rvSetDesk && window.rvSetDesk(\(deskIsDark));")
        }
        if c.lastTool != tool {
            c.lastTool = tool
    ...
    final class Coordinator: NSObject, WKScriptMessageHandler, WKNavigationDelegate, WKUIDelegate {
        ...
        func adopt(_ view: DocumentWebView) {
            ...
            pendingDeskIsDark = view.deskIsDark
        }
        ...
        func userContentController(_ controller: WKUserContentController,
            ...
            case "ready":
                ...
                lastDeskIsDark = pendingDeskIsDark
            case "selection":
