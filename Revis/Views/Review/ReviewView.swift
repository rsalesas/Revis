...
struct ReviewView: View {
    ...
    /// The reviewer's appearance, for the desk alone. The document's own web view is pinned
    /// to a light one so a document with a dark branch does not take it against white paper
    /// — see `DocumentWebView` — which leaves the desk needing to be told.
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            UpdateBar(checker: appState.updateChecker)
            content
